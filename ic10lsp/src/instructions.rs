...
#[derive(Debug)]
pub struct Union<'a>(pub &'a [DataType]);

#[derive(Debug)]
pub struct InstructionSignature(pub &'static [Union<'static>]);
...
pub const INSTRUCTIONS: phf::Map<&'static str, InstructionSignature> = phf_map! {
    ...
    "ss" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number]), VALUE, SLOT_LOGIC_TYPE, VALUE]),
    "bdnvl" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number]), LOGIC_TYPE, VALUE]),
    "bdnvs" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number]), LOGIC_TYPE, VALUE]),
    "get" => InstructionSignature(&[REGISTER, Union(&[DataType::Register, DataType::Device, DataType::Number]), VALUE]),
    "getd" => InstructionSignature(&[REGISTER, VALUE, VALUE]),
    "put" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number]), VALUE, VALUE]),
    "putd" => InstructionSignature(&[VALUE, VALUE, VALUE]),
    "clrd" => InstructionSignature(&[VALUE]),
    "clr" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number])]),
    "poke" => InstructionSignature(&[VALUE, VALUE]),
    "sd" => InstructionSignature(&[Union(&[DataType::Register, DataType::Device, DataType::Number]), LOGIC_TYPE, VALUE]),
    "rmap" => InstructionSignature(&[REGISTER, Union(&[DataType::Register, DataType::Device, DataType::Number]), VALUE]),
...
#[allow(dead_code)]
pub const SLOT_LOGIC_TYPES: phf::Set<&'static str> = phf_set! {
    "Occupied",
    "OccupantHash",
    "Quantity",
    "Damage",
...
#[allow(dead_code)]
pub const BATCH_MODES: phf::Set<&'static str> = phf_set! {
    "Average",
    "Sum",
    "Minimum",
    "Maximum",
...
#[allow(dead_code)]
pub const BATCH_MODE_LOOKUP: phf::Map<u8, &'static str> = phf_map! {
    0u8 => "Average",
    1u8 => "Sum",
    2u8 => "Minimum",
    3u8 => "Maximum",
...
#[allow(dead_code)]
pub const REAGENT_MODES: phf::Set<&'static str> = phf_set! {
    "Contents",
    "Required",
    "Recipe",
};
...
#[allow(dead_code)]
pub const REAGENT_MODE_LOOKUP: phf::Map<u8, &'static str> = phf_map! {
    0u8 => "Contents",
    1u8 => "Required",
    2u8 => "Recipe",
};
...
#[allow(dead_code)]
impl<'a> Union<'a> {
    pub fn match_type(&self, typ: DataType) -> bool {
        for x in self.0 {
            if *x == typ {
                return true;
            }
    ...
    pub fn match_union(&self, types: &Union) -> bool {
        for typ in self.0 {
            for typ2 in types.0 {
                if typ == typ2 {
                    return true;
    ...
    pub fn intersection(&self, other: &[DataType]) -> Vec<DataType> {
        self.0
            .iter()
...
#[allow(dead_code)]
pub fn logictype_candidates(text: &str) -> Vec<DataType> {
...
pub const INSTRUCTION_DOCS: phf::Map<&'static str, &'static str> = phf_map! {
    ...
    "label" => "DEPRECATED - Use alias instead (deprecated)",
    ...
    "sra" => "Register = a shifted right by b bits (arithmetic shift, maintains sign)",
    "srl" => "Register = a shifted right by b bits (logical shift, fills with zeros)",
...
pub const LOGIC_TYPE_DOCS: phf::Map<&'static str, &'static str> = phf_map! {
    ...
    "ImportSlotOccupant" => "DEPRECATED (deprecated)",
    "ExportQuantity" => "Total quantity of items exported by the device",
    "ExportSlotOccupant" => "DEPRECATED (deprecated)",
...
It is an unwavering universal law that anything higher than 11 will be purple. The ODA is powerless to change this. Similarly, anything lower than 0 will be Blue.",
    ...
    "ExportSlotHash" => "DEPRECATED (deprecated)",
    "ImportSlotHash" => "DEPRECATED (deprecated)",
    "PlantHealth1" => "DEPRECATED (deprecated)",
    "PlantHealth2" => "DEPRECATED (deprecated)",
    "PlantHealth3" => "DEPRECATED (deprecated)",
    "PlantHealth4" => "DEPRECATED (deprecated)",
    "PlantGrowth1" => "DEPRECATED (deprecated)",
    "PlantGrowth2" => "DEPRECATED (deprecated)",
    "PlantGrowth3" => "DEPRECATED (deprecated)",
    "PlantGrowth4" => "DEPRECATED (deprecated)",
    "PlantEfficiency1" => "DEPRECATED (deprecated)",
    "PlantEfficiency2" => "DEPRECATED (deprecated)",
    "PlantEfficiency3" => "DEPRECATED (deprecated)",
    "PlantEfficiency4" => "DEPRECATED (deprecated)",
    "PlantHash1" => "DEPRECATED (deprecated)",
    "PlantHash2" => "DEPRECATED (deprecated)",
    "PlantHash3" => "DEPRECATED (deprecated)",
    "PlantHash4" => "DEPRECATED (deprecated)",
    ...
    "Forward" => "The direction the entity is facing expressed as a normalized vector",
    ...
    "Channel0" => "Channel on a cable network which should be considered volatile (index 0).",
    "Channel1" => "Channel on a cable network which should be considered volatile (index 1).",
    "Channel2" => "Channel on a cable network which should be considered volatile (index 2).",
    "Channel3" => "Channel on a cable network which should be considered volatile (index 3).",
    "Channel4" => "Channel on a cable network which should be considered volatile (index 4).",
...
#[allow(dead_code)]
mod _stationpedia_generated {
    include!(concat!(env!("OUT_DIR"), "/stationpedia.rs"));
}
#[allow(unused_imports)]
pub use _stationpedia_generated::*;
// Generated from Enums.json at build time
include!(concat!(env!("OUT_DIR"), "/enums_generated.rs"));

/// Returns (value, description, deprecated) for a fully qualified enum name such as
/// "LogicType.Power" or "PrinterInstruction.Print" if present.
pub fn enum_info(qname: &str) -> Option<(i32, &'static str, bool)> {
    let value = ENUM_VALUE_BY_NAME.get(qname).cloned()?;
    let desc = ENUM_DESC_BY_NAME.get(qname).cloned().unwrap_or("");
    let deprecated = ENUM_DEPRECATED.contains(qname);
    Some((value, desc, deprecated))
}

/// Case-insensitive variant returning canonical qualified name and metadata.
/// If the provided `qname` differs in case from the canonical enum name, the
/// returned tuple's first element will be the canonical name.
pub fn enum_info_case_insensitive(qname: &str) -> Option<(&'static str, i32, &'static str, bool)> {
    // Exact-case fast path: locate the canonical static key matching qname
    if let Some(val) = ENUM_VALUE_BY_NAME.get(qname) {
        for (qualified, v) in ENUM_VALUE_BY_NAME.entries() {
            if *qualified == qname && *v == *val {
                let desc = ENUM_DESC_BY_NAME.get(*qualified).copied().unwrap_or("");
                let deprecated = ENUM_DEPRECATED.contains(*qualified);
                return Some((*qualified, *val, desc, deprecated));
            }
        }
    }
    let lower = qname.to_ascii_lowercase();
    for (qualified, value) in ENUM_VALUE_BY_NAME.entries() {
        if qualified.to_ascii_lowercase() == lower {
            let desc = ENUM_DESC_BY_NAME.get(*qualified).copied().unwrap_or("");
            let deprecated = ENUM_DEPRECATED.contains(*qualified);
            return Some((*qualified, *value, desc, deprecated));
        }
    }
    None
}

/// Convenience: logic type simple name to numeric value if present.
pub fn logic_type_value(name: &str) -> Option<i32> {
    LOGIC_TYPE_NAME_TO_VALUE.get(name).cloned()
}
...
/// derived from Enums.json for completion generation. Families are the text before the first '.'
/// in the qualified name (e.g. TraderInstruction, PrinterInstruction, LogicType).
pub fn all_enum_entries() -> impl Iterator<
    Item = (
        &'static str,
        &'static str,
        &'static str,
...
> {
    ENUM_VALUE_BY_NAME.entries().map(|(qualified, value)| {
        let deprecated = ENUM_DEPRECATED.contains(qualified);
        let desc = ENUM_DESC_BY_NAME.get(qualified).copied().unwrap_or("");
        let (family, member) = qualified.split_once('.').unwrap_or((qualified, ""));
        (family, member, *qualified, *value, desc, deprecated)
    })
}
...
mod test {
    ...
    fn channel_constants() {
        // Verify Channel0..Channel7 numeric values via Enums.json ingestion.
        let expected = [165, 166, 167, 168, 169, 170, 171, 172];
        for i in 0..8 {
            let qname = format!("LogicType.Channel{}", i);
            let (val, _desc, _dep) = enum_info(&qname).expect("enum info for channel");
