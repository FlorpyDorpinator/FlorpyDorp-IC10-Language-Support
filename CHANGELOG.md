...
- **Context-Aware Filtering**: Completions now filter based on instruction parameter types
  ...
  - Register parameters (e.g., `add r0 _`) only show registers and register aliases
  - Device parameters only show device references and aliases
...
- **Device Completions in HASH()**: Revolutionary dropdown experience
  - Typing `HASH("")` now shows all 1709 device names with fuzzy filtering
...
- **HASH() Number Validation**: New error diagnostic
  - `HASH("123")` now shows ERROR diagnostic with message
